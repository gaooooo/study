    ...
    jQuery.fn.imageMaps = function(setting) {
		...
		$container.each(function(){
			...
			$images.each(function(){
				//image-maps-conrainer容器
				var _img_conrainer = $(this).parent();
				_img_conrainer.append('<div class="button-conrainer"><input type="button" value="Add Link" /></div>')
					.append('<div class="link-conrainer"></div>')
					.append($.browser.msie ? $('<div class="position-conrainer" style="position:absolute"></div>')
						.css({
					background:'#fff',
					opacity:0
				...
				if(map_name !=''){
					...
					container.find('map[name='+map_name+']').find('area[shape=rect]').each(function(){
						...
						coords = coords.split(',');
						_position_conrainer.append('<div ref="'+index+'" class="map-position" style="left:'+coords[0]+'px;top:'+coords[1]+'px;width:'+(coords[2]-coords[0])+'px;height:'+(coords[3]-coords[1])+'px;">' +
							'<div class="map-position-bg"></div>' +
							'<span class="link-number-text">Link '+index+'</span>' +
							'<span class="delete">X</span>' +
							'<span class="resize"></span></div>');
						index++;
					});
				}
			});
